(function () {
  ...
  function setNumber(digit) {
    ...
    !output.dataset.operator
      ? printNumber(digit, "value1")
      : printNumber(digit, "value2");
  }
  ...
  function setOperator(currentOperator) {
    ...
    const isLastKey4thOperator =
      operator4thArray.indexOf(output.dataset.lastkey) > -1; // 最后一个键是算术运算符
    const isCurrentKey4thOperator =
      operator4thArray.indexOf(currentOperator) > -1; // 当前键为算术运算符
